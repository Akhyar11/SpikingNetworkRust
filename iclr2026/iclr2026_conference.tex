...
\begin{itemize}
    ...
    \item \textbf{Heterogeneous Neuron Dynamics}: The integration of variable, trainable leak and threshold parameters within Leaky Integrate-and-Fire (LIF) neurons, enhancing the semantic representational capacity of the discrete spike domain.
...
After passing through the spiking attention layers, the output remains a sequence of spike trains $S^{(t)}$. To derive a static, continuous sentence embedding vector suitable for evaluating semantic similarity, we implement temporal pooling using a frozen Identity Spiking BPTT layer. Unlike a raw algebraic sum, this layer maintains stateful LIF dynamics but initializes and freezes its recurrent weights as an identity matrix ($W = I$). The final embedding $E$ is derived by accumulating the membrane potentials $U_{pool}$ of this frozen layer over the sequence length $L$:
\begin{equation}
...
To bypass the challenges of applying contrastive learning directly on discrete spikes, we utilize knowledge distillation from a pre-trained dense Transformer (e.g., MiniLM). Instead of traditional continuous loss functions (e.g., MSE or Cosine Embedding Loss), the SNN is trained using an event-driven Contrastive Hebbian learning rule. The learning signal is driven by target similarity scores derived from the teacher model. If the teacher indicates high semantic similarity between a pair, the Hebbian rule initiates a "pull" (Long-Term Potentiation) to co-activate related neurons. Conversely, low similarity triggers a "push" (Long-Term Depression):
\begin{equation}
\Delta W_{ij} \propto S_i^{pre} \times ( \text{Sim}_{teacher} - \text{margin} )
\end{equation}
To further address the training instability inherent to local learning rules, we implement Homeostatic Weight Clamping during the learning loop. This explicitly prevents weight explosion and "dead neuron" syndrome, ensuring stable propagation of semantic gradients.
...
\textbf{Hyperparameters and Controls.} To ensure strict, deterministic reproducibility and eliminate initialization bias, all models are trained from identical seeded weight initializations. The architecture utilizes a hidden dimension $d=64$, a maximum sequence length $L=32$, and processes spikes over $T=8$ time steps.
