...
\end{equation}
This operation acts as a stateful Identity Integrator during inference, preserving the sequence-level semantic topology while maintaining biologically plausible integration dynamics. To ensure the purity of this integration, the pooling layer is explicitly frozen during training, acting purely as an unbiased temporal accumulator without any learned recurrent bias projections.
...
To guarantee convergence and protect the acquired semantic topology, we implement two critical optimizations to the Hebbian update rule. First, we introduce a continuous error scaling margin ($m=0.05$). Rather than utilizing a hard threshold, this margin acts as a proportional multiplier to the local prediction error, ensuring that synaptic updates gracefully scale down as the network approaches the stable equilibrium of the continuous semantic manifold. Second, we integrate a Cosine Annealing learning rate schedule across the training epochs. The learning rate is smoothly decayed following a cosine curve, which suppresses aggressive weight perturbations in the final stages of distillation and firmly locks in the transferred topology.
