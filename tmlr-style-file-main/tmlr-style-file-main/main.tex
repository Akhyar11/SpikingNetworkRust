...
\end{equation}
During Backpropagation Through Time, the error $\delta^{(t)}$ at sequence step $t$ is explicitly linked to future time steps via the heterogeneous decay $\bm{\beta}_{pool}$:
\begin{equation}
...
To facilitate reproducibility and open science, the fully trained versions of our Attention-Free Spiking Sentence Embedder ($d_{model} = 256$) have been publicly released. The model weights, PyTorch-compatible SNN architecture scripts, and configuration files are available online.
\begin{itemize}
    \item \textbf{Base Model}: Achieves the reported $r = 0.8030$ Pearson correlation on the STS benchmarks.
    \item \textbf{Multilingual Model}: Extends the vocabulary to $64,000$ to support cross-lingual semantic search across 20+ languages.
\end{itemize}
\textit{Note: The direct URL links to the Hugging Face repository have been temporarily redacted to maintain anonymity during the double-blind review process. They will be fully restored in the camera-ready version.}
