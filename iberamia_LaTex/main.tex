...
\input{../tmlr_submition/math_commands.tex}
...
% Obfuscate metadata for blind review
\renewcommand{\thevolume}{XX}
\renewcommand{\theissue}{XX}
\renewcommand{\theyear}{XXXX}

\title{Delta BPTT: Efficient Add-Only Backpropagation Through Time for Spiking Sequence Representations\\
\vspace{0.5cm}
\emph{Delta BPTT: Retropropagación Eficiente a través del Tiempo con Operaciones Solo de Suma para Representaciones de Secuencias de Picos}
}

%       The journal uses a double-blind review processe
\author{Anonymous Authors
\smallskip \\
\small{Anonymous Affiliations\\
...
Las Redes Neuronales de Picos (SNNs) proporcionan una alternativa energéticamente eficiente a las Redes Neuronales Profundas convencionales mediante la utilización de picos temporales discretos impulsados por eventos. Sin embargo, el entrenamiento de SNNs en tareas temporales complejas mediante la Retropropagación a través del Tiempo (BPTT) a menudo anula estas ganancias de eficiencia: el paso continuo hacia atrás reintroduce operaciones densas de Multiplicación y Acumulación (MAC) de punto flotante durante la acumulación de gradientes de parámetros. En este artículo, presentamos \textbf{Delta BPTT}, un algoritmo de enrutamiento de gradiente de solo suma que explota la naturaleza estrictamente binaria de los eventos de picos directos para reducir la acumulación densa de gradientes de peso en sumas condicionales y dispersas. Al formular una Máscara Rectangular Sustituta sobre el desenrollamiento temporal y controlar las actualizaciones de parámetros exclusivamente a través de funciones indicadoras de picos binarios $\mathbb{I}[S^{(t)}=1]$, Delta BPTT elimina las operaciones densas de MAC tanto en la proyección hacia adelante recurrente como en las fases de acumulación de gradientes de parámetros. Evaluamos bajo dos protocolos complementarios: fidelidad de destilación (correlación de Pearson frente a puntuaciones del modelo maestro), donde el modelo de $65$K parámetros alcanza $0.8031$ en el agregado ALL-STS de $29{,}720$ pares, y el rendimiento en tareas zero-shot frente a etiquetas humanas de referencia siguiendo la convención MTEB, donde alcanza $0.7651$ Pearson / $0.7659$ Spearman en STS-B, superando a los modelos base de incrustaciones estáticas (GloVe, Word2Vec) utilizando aproximadamente tres órdenes de magnitud menos parámetros que modelos de lenguaje de picos como SpikingBERT ($\sim 50$M). Reemplazar el complejo enrutamiento de atención espacial con una agrupación temporal puramente recurrente entrenada mediante Delta BPTT ofrece una reducción de $4.13\times$ en el tiempo de entrenamiento ($1{,}165$\,s vs $4{,}817$\,s) y una reducción de $2.09\times$ en la latencia de inferencia, demostrando que los gradientes temporales de solo suma estructuralmente optimizados ofrecen una alternativa altamente eficiente al enrutamiento espacial explícito en la representación de secuencias neuromórficas.}}

\medskip

\noindent{\small{\textbf{Keywords}: Spiking Neural Networks, Sequence Representation, Backpropagation Through Time, Energy Efficiency.}}

\noindent{\small{\textbf{Palabras Clave}: Redes Neuronales de Picos, Representación de Secuencias, Retropropagación a través del Tiempo, Eficiencia Energética.}}

\section{Introduction}

The computational overhead of Dense Sentence Embedders \cite{devlin2018bert}, such as SBERT \cite{reimers2019sentence}, is heavily dominated by the dense matrix multiplications required in the standard Scaled Dot-Product Attention introduced by \cite{vaswani2017attention}. While numerous efficient Transformer architectures have been proposed to mitigate this quadratic $\mathcal{O}(L^2)$ complexity \cite{tay2022efficient}, they still rely fundamentally on continuous floating-point operations. Spiking Neural Networks (SNNs) alleviate this constraint by operating on sparse, event-driven additions rather than complex multiplications, making them highly suitable for highly parallel neuromorphic hardware \cite{roy2019towards}. Recent neuromorphic NLP research has focused on translating Self-Attention to SNNs \cite{li2022spikeformer, zhu2023spikegpt} and extending these architectures to extract semantic embeddings. Furthermore, the development of specialized frameworks like SpikingJelly \cite{fang2023spikingjelly} has accelerated the validation of surrogate gradient methods in deep architectures.
...
To optimize these dynamics, we distill ``dark knowledge'' from a pre-trained dense Teacher, adapting established knowledge distillation techniques \cite{hinton2015distilling} to the discrete spiking domain. We utilize a Mean Squared Error (MSE) objective over the Pearson Correlation scores. The exact error $\mathcal{E}$ is given by:
\begin{equation}
...
\begin{table}[t!]
\begin{center}
{\caption{Theoretical complexity and operation count comparison per sequence of length $L$ and model dimension $D$. $s \in [0,1]$ denotes the forward spike sparsity ($s \approx 0.67$). Energy is computed using 45\,nm CMOS proxies ($E_{\text{MAC}} = 4.6$\,pJ, $E_{\text{AC}} = 0.9$\,pJ).}\label{tab:complexity}}
...
\begin{table}[t!]
\begin{center}
{\caption{Hardware specifications for training and inference.}\label{tab:hardware_specs}}
...
\begin{table}[t!]
\begin{center}
{\caption{Synthetic Knowledge Distillation dataset and training specifications.}\label{tab:dataset_specs}}
...
\begin{table}[t!]
\begin{center}
{\caption{Comprehensive evaluation: dimensionality scaling and architectural ablation.}\label{tab:main_results}}
...
Using the Distillation strategy with the Delta BPTT Recurrent Pooler, we scaled the embedding dimension $D_{model}$ to analyze representation capacity. The SNN scales consistently up to 256 dimensions, crossing the 0.80 Pearson correlation barrier, which represents a substantial improvement over prior Spike Coincidence Attention baselines \cite{coincidence2026} ($r=0.758$).
...
To situate our results within the broader landscape of sentence embedding models, Table~\ref{tab:external_baselines} presents a cross-model comparison using published results from the respective original papers. We report STS-B Spearman correlation as the standard metric following the MTEB convention \cite{muennighoff2022mteb}.

\begin{table}[t!]
\begin{center}
{\caption{Comparison with published baselines on STS-B (Spearman correlation). SNN and Transformer results are taken from their respective original papers. $^\dagger$Results from \cite{bal2024spikingbert}. $^\ddagger$Results from \cite{zhou2025spikingformer}. $^\star$Results from \cite{sanh2019distilbert}. Static embedding baselines use mean-pooled 300d vectors.}\label{tab:external_baselines}}
...
\begin{table}[t!]
\begin{center}
{\caption{Forward and backward pass sparsity ratios measured during training at $D_{model} = 256$. Backward sparsity is computed over the weight gradient accumulation of the Delta BPTT Pooler layer.}\label{tab:sparsity}}
...
\begin{table}[t!]
\begin{center}
{\caption{Empirical scaling behavior across sequence lengths $L \in \{32, 64, 128, 256, 512\}$ at $D_{model}=256$. Latency is measured per sentence pair on CPU.}\label{tab:seq_length_scaling}}
...
\begin{table}[t!]
\begin{center}
{\caption{Component ablation of the Delta BPTT architecture ($D_{model}=256$).}\label{tab:component_ablation}}
...
\bibliography{../tmlr_submition/main}
