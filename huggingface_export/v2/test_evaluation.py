...
def evaluate():
    ...
    with torch.no_grad():
        for i in range(0, len(test_data), batch_size):
            ...
            # PAD token → 0 karena di Rust lookup dimulai dari index 0 (tidak ada embedding untuk 0)
            ...
            # Model sudah menghasilkan embedding mean-centered + unit-variance (z-score),
            # jadi cosine similarity langsung = dot product = Pearson correlation.
            sims = F.cosine_similarity(embs1, embs2).clamp(min=0.0).tolist()
