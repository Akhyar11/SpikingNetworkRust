...
impl SpikingSentenceEmbedder {
    ...
    /// Melatih jaringan menggunakan Greedy Layer-Wise Contrastive Learning persis seperti versi JS.
    pub fn train_step(&mut self, texts: &[&str], num_pairs: usize, margin: f32) -> f32 {
        self.zero_pad_token();
        let batch_size = texts.len();
        self.embedding.reset_state();
        ...
        SpikingNetworkRust::core::contrastiveHebbian::contrastiveHebbian(
            &spikes2, &mut err_att_data, num_pairs, self.max_seq_length, d_model, margin
        );
        ...
        let pooler_loss = SpikingNetworkRust::core::contrastiveHebbian::contrastiveHebbian(
            &normalized_out_data, &mut error_final_data, num_pairs, 1, self.pooler.units, margin
        );
