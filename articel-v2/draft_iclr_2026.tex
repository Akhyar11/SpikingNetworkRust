...
\title{\centering Is Spike-Driven Self-Attention Necessary? The Inefficiency\\ of Spike-Overlap Attention in Spiking\\ Sentence Embeddings}

\author{\end{tabular}\hfill\begin{tabular}[t]{c}\bf\rule{0pt}{24pt}Muhammad Akhyar \\ \normalfont\texttt{akhyarsafrudin@gmail.com}\end{tabular}\hfill\begin{tabular}[t]{l}}
...
\begin{enumerate}
    ...
    \item We introduce Add-Only Delta BPTT routing, significantly reducing floating-point matrix multiplications during the backward pass of the recurrent temporal pooler.
...
\begin{tikzpicture}[
    font=\sffamily,
    base/.style={rectangle, rounded corners, draw=black, thick, minimum height=3em, align=center, fill=white},
    input/.style={base, text width=18em},
    spike/.style={base, text width=10em},
    pooler/.style={base, text width=12em, minimum height=4em},
    attn/.style={base, text width=12em, minimum height=4em},
    output/.style={base, text width=10em},
    arrow/.style={-latex, thick, draw=black},
    dashed_arrow/.style={-latex, dashed, thick, draw=black}
]

% Center column
\node[input] (tokens) at (0, 0) {\textbf{Tokenized Input Sequence} \\ \small \texttt{[CLS] The quick brown ... [PAD]}};
\node[spike] (lif) at (0, -2) {\textbf{Spike Encoding} \\ \small Heterogeneous LIF ($V^{(t)}$)};
...
\node[attn] (attention) at (4.5, -5.5) {\textbf{Baseline: Spike-Overlap Attn} \\ \small $O(L^2)$ Boolean Intersection \\ \small \textit{Spatial routing}};

% Split paths
\draw[arrow] (lif) -- (0, -3.5) -| node[above, pos=0.25, font=\small, text=black] {Spikes $S^{(t)}$} (recurrent);
\draw[arrow] (lif) -- (0, -3.5) -| node[above, pos=0.25, font=\small, text=black] {Spikes $S^{(t)}$} (attention);

% Output
\node[output] (out) at (0, -8.5) {\textbf{Continuous Embedding} \\ \small $\tilde{E} \in \mathbb{R}^{D_{model}}$};

% Join paths
\draw[arrow] (recurrent) |- node[above, pos=0.75, font=\small, text=black] {Accumulate $\sum V_{pool}^{(t)}$} (out);
\draw[arrow] (attention) |- node[above, pos=0.75, font=\small, text=black] {Accumulate $\sum V_{att}^{(t)}$} (out);

% Annotations
\node[align=left, font=\scriptsize, text=black] (note1) at (5, -2) {\textbullet~ Soft-Reset Dynamics \\ \textbullet~ Hyperpolarized Padding};
\draw[dashed_arrow] (note1) -- (lif);

\node[align=right, font=\scriptsize, text=black] (note2) at (-4.5, -3.8) {\textbullet~ Identity Init \\ \textbullet~ High Sparsity};
\draw[dashed_arrow] (note2) -- (recurrent);

\node[align=left, font=\scriptsize, text=black] (note3) at (4.5, -3.8) {\textbullet~ XOR Integration ($\oplus$) \\ \textbullet~ Double LIF Gates};
...
\begin{equation}
V_{emb}^{(t)} = \min\left(1.0, \beta_{emb} \odot V_{emb}^{(t-1)} + X^{(t)} W_{emb}\right) - S_{emb}^{(t)} \odot \theta_{emb}
\end{equation}
...
\textbf{Spike-Difference (XOR) Integration:} Following the generation of the attention-gated spikes (denoted as $S_{att}^{(t)}$), we integrate these contextual features with the original embedding spikes $S_{emb}^{(t)}$ using an exclusive-OR (XOR) logical operation: 
\begin{equation}
S_{agg}^{(t)} = S_{emb}^{(t)} \oplus S_{att}^{(t)}
\end{equation}
...
\node[base, text width=12em] (semb) at (0, 0) {\textbf{Embedding Spike $S_{emb}^{(t)}$}};
\node[base] (k) at (0, -2) {Key LIF ($K$)};
\node[base] (q) at (-4, -2) {Query LIF ($Q$)};
\node[base] (v) at (4, -2) {Value LIF ($V$)};

\draw[arrow] (semb) -- (k);
\draw[arrow] (semb) -| (q);
\draw[arrow] (semb) -| (v);

\node[base, text width=16em] (intersect) at (0, -4) {Boolean Intersection \\ $\sum_{d=1}^{D} \left( Q_{i,d}^{(t)} \wedge K_{j,d}^{(t)} \right)$};
\draw[arrow] (q) |- (intersect);
\draw[arrow] (k) -- (intersect);

\node[base] (norm) at (0, -6) {Normalize by Max Matches};
\draw[arrow] (intersect) -- (norm);

\node[base] (aux) at (0, -8) {Auxiliary Attention LIF};
\draw[arrow] (norm) -- (aux);

\node[base, text width=14em] (xor) at (0, -10) {XOR Integration ($\oplus$)};
\draw[arrow] (aux) -- node[right] {$S_{att}$} (xor);
\draw[arrow] (semb.west) -- ++(-5.5,0) |- (xor.west);

\node[base, text width=14em] (gate) at (0, -12) {Value Gating};
\draw[arrow] (xor) -- node[right] {$S_{agg}$} (gate);
\draw[arrow] (v) |- (gate);

\node[base] (out) at (0, -14) {\textbf{Attention Output}};
\draw[arrow] (gate) -- (out);
...
\begin{equation}
X_{pool}^{(t)} = \sum_{d=1}^{D_{in}} W_{d} \cdot \mathbb{I}\left[ S_{emb, d}^{(t)} = 1 \right]
\end{equation}
...
\begin{equation}
\sigma'\big(V^{(t)}\big) = \begin{cases} 1 & \text{if } \big|V^{(t)} - \theta\big| < w \\ 0 & \text{otherwise} \end{cases}
\end{equation}
...
\begin{equation}
\tilde{\delta}^{(t)} = \left( \delta_{out}^{(t)} + \tilde{\delta}^{(t+1)} \odot \beta_{pool} \right) \odot \sigma'\big(V_{pool}^{(t)}\big)
\end{equation}
...
\begin{equation}
\Delta W_{in, out} = \sum_{t=1}^{T} \tilde{\delta}^{(t)}_{out} \cdot \mathbb{I}\left[ S_{emb, in}^{(t)} = 1 \right]
\end{equation}
...
\node[base, text width=16em] (semb) at (0,0) {\textbf{Input Spike $S_{emb}^{(t)} \in \{0,1\}$}};
\node[base] (cond) at (0, -2) {Condition: $S_{emb}^{(t)} == 1$};
\draw[arrow] (semb) -- (cond);

\node[base, text width=11em] (add) at (-3, -4) {Add Weight $W_{d}$ to $V_{pool}$};
\node[base, text width=10em] (skip) at (3, -4) {Skip (No Operation)};

\draw[arrow] (cond) -| node[above left] {Yes} (add);
\draw[arrow] (cond) -| node[above right] {No} (skip);

\node[base, text width=14em] (vpool) at (0, -6) {Membrane Potential $V_{pool}^{(t)}$};
\draw[arrow] (add) |- (vpool);
\draw[arrow] (skip) |- (vpool);

\node[base] (decay) at (0, -8) {Temporal Decay $\beta$};
\draw[arrow] (vpool) -- (decay);

\node[base] (acc) at (0, -10) {Accumulate over sequence $T$};
\draw[arrow] (decay) -- (acc);

\node[base, text width=16em] (out) at (0, -12) {\textbf{Final Continuous Embedding}};
\draw[arrow] (acc) -- (out);
...
\begin{equation}
\mathcal{E} = \text{Sim}_{Teacher} - \sum_{d=1}^{D_{model}} \left(\tilde{E}_{1,d} \cdot \tilde{E}_{2,d}\right)
\end{equation}
where $\tilde{E}$ denotes the L2 unit-norm normalized, mean-centered pooled embeddings. The error gradient is derived symmetrically across the representation space:
\begin{align}
\frac{\partial \mathcal{L}}{\partial \tilde{E}_{1,d}} &= -\mathcal{E} \cdot \tilde{E}_{2,d} \cdot m \\
\frac{\partial \mathcal{L}}{\partial \tilde{E}_{2,d}} &= -\mathcal{E} \cdot \tilde{E}_{1,d} \cdot m
\end{align}
...
To facilitate reproducibility and open science, the fully trained version of our Attention-Free Spiking Sentence Embedder ($d\_model = 256$), which achieves the reported $r = 0.8030$ Pearson correlation on the STS benchmarks, has been publicly released. The model weights, PyTorch-compatible SNN architecture scripts, and configuration files are available on Hugging Face at \url{https://huggingface.co/PulseNet-Labs/spiking-sentence-embedder-v2}.
